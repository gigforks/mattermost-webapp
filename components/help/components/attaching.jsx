...
export default function HelpAttaching() {
    ...
    message.push(localizeMessage('help.attaching.previewer', '## File Previewer\TF Chat has a built in file previewer that is used to view media, download files and share public links. Click the thumbnail of an attached file to open it in the file previewer.'));
    message.push('\n');
    message.push(localizeMessage('help.attaching.publicLinks', '#### Sharing Public Links\nPublic links allow you to share file attachments with people outside your TF Chat team. Open the file previewer by clicking on the thumbnail of an attachment, then click **Get Public Link**. This opens a dialog box with a link to copy. When the link is shared and opened by another user, the file will automatically download.'));
    ...
    message.push(localizeMessage('help.attaching.supported', '#### Supported Media Types\nIf you are trying to preview a media type that is not supported, the file previewer will open a standard media attachment icon. Supported media formats depend heavily on your browser and operating system, but the following formats are supported by TF Chat on most browsers:'));
    ...
    message.push(localizeMessage('help.attaching.limitations', '## File Size Limitations\TF Chat supports a maximum of five attached files per post, each with a maximum file size of 50Mb.'));
